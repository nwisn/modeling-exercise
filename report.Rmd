...
# read in new data
newdatafile <- "mynewdata.csv" # whatever you got
newdata <- read.csv(newdatafile)

# apply same pre-processing to features
testing <- predict(full$preProcess.model, newdata) 

# get all model predctions
predictions <- sapply(finalmodel.cv, 
                      function(this) predict(this$model, testing))

# estimate error stats
require(caret)
performance <- apply(predictions, 2, 
                     function(pred) postResample(pred, newdata$Dependent_Feature))
```
